...
\multauthor
  {Yining Yang$^{1*}$ \hspace{0.5cm} Ruogu Chen$^{3*}$ \hspace{0.5cm} Kate Helsen$^1$ \hspace{0.5cm} Mark Daley$^2$ \hspace{0.5cm} Jie Han$^3$}
  {$^1$ Don Wright Faculty of Music, Western University, Canada\\
   ...
   {\tt\small yyang524@uwo.ca, ruogu@ualberta.ca, kate.helsen@uwo.ca, mdaley@uwo.ca, jhan8@ualberta.ca}\\[2pt]
   \small $^*$Equal contribution}


% Camera-ready value:
\def\authorname{Y.\ Yang, R.\ Chen, K.\ Helsen, M.\ Daley, and J.\ Han}
...
First, Region of Interest (ROI) Align \cite{he2017mask} extracts a fixed-size feature patch $f_r$ from the stride-8 feature map:
\begin{equation}\label{eq:roi}
f_r = \mathrm{ROIAlign}(F,\, r).
\end{equation}
The extracted features $f_r$ are then modulated by the audio vector~$z_t$ via Feature-wise Linear Modulation (FiLM) \cite{perez2018film}:
...
\bar{f}_r = \mathrm{softmax}\!\left(\frac{\mathrm{flat}(\hat{f}_r)\,(W_a H_t)^\top}{\sqrt{C}}\right) W_a H_t,
\end{equation}
where $W_a$ is a learned linear projection onto the $C$-dimensional attention space shared with~$\hat{f}_r$. This cross-attention lets each candidate region attend to fine-grained temporal patterns in the recent audio history.

The system stage applies Eqs.~\ref{eq:roi}--\ref{eq:crossattn} to every system candidate on the page, producing~$\log p(s_t \mid h_t)$ via adaptive average pooling, a linear classifier, and softmax normalization. The bar stage applies the same pipeline with independent parameters to the bars within the selected system, yielding~$\log p(b_t \mid s_t, h_t)$. The note stage uses only Eqs.~\ref{eq:roi}--\ref{eq:conv} (no cross-attention at this stage) on the selected bar and regresses bar-local coordinates~$u_t = (c_x, c_y) \in [0,1]^2$ via sigmoid activation.
...
To fairly evaluate jump recovery, we construct a jump test benchmark from the 94 MSMD test pieces. We manually annotated the repeat structure of each piece (repeat barlines, da capo, volta brackets, binary form, etc.). Of the 94 pieces, 66 contain written repeat structures and 28 do not. The benchmark is partitioned into two subsets accordingly. The \emph{repeat subset} comprises the 66 pieces with repeats, producing 131 jumps that follow the annotated performance order. The \emph{random subset} comprises the remaining 28 pieces without repeats, each receiving at least 3 randomly placed jumps as a stress test.
...
\begin{table}[h]
\centering
\caption{Jump recovery results on the repeat-aware jump-augmented MSMD test set.
\textbf{Rec.@$k$\,s}: fraction of jumps where the correct system is identified
within $k$ seconds after audio resumes.
\textbf{Lat.}: mean time in seconds from audio resumption to first correct system prediction.
\textbf{Post-J.}: fraction of onsets tracked within ${\leq}\,1.0$\,s in the 5\,s window
following each jump (standard tracking accuracy applied to post-jump segments).}
\label{tab:jump}
\footnotesize
...
Table~\ref{tab:jump} shows the results. Without a jump-handling mechanism, CYOLO-SB yields low recovery rates across both subsets. CODA without break mode also struggles because learned transition penalties resist large position changes. CODA with break mode substantially improves recovery. On the repeat subset, CODA recovers the correct system within 1\,s in 78\% of jumps. Recovery is higher on the repeat subset than the random subset (.78 vs.\ .64 at 1\,s), indicating that random jumps are more difficult to handle than patterned written discontinuities.
...
Table~\ref{tab:ablation} shows the results. The largest degradation comes from removing the cascade ($-.045$ at ${\leq}\,0.10$\,s, bar accuracy drops from .975 to .931), confirming that enforced geometric consistency is the most important component. Beam search and cross-attention contribute the next-largest gains ($-.026$ and $-.019$ at ${\leq}\,0.10$\,s, respectively). Scheduled sampling has the smallest individual effect, but remains necessary to close the train-inference routing gap.
