...
Let $H_t = [h_{t-W+1}, \ldots, h_t] \in \mathbb{R}^{W \times d}$ denote the buffer of the most recent~$W$ Mamba hidden states, where $d$ is the hidden dimension of the Mamba encoder. For each candidate region~$r$ (a system or bar bounding box from the score layout metadata), the pipeline proceeds as follows.
...
Table~\ref{tab:jump} shows the results. CYOLO-SB has no recovery mechanism and
continues from its last committed position, yielding low recovery rates on both
subsets. CODA without break mode also struggles because learned transition
penalties resist large position changes. Enabling break mode substantially
improves recovery by suppressing priors and widening the beam during detected
silence. On the repeat subset, which tests musically realistic discontinuities,
CODA recovers the correct system within 1\,s in [--]\% of jumps.
...
This paper presented CODA, a real-time score following system that formulates image-based tracking as cascaded selection over known system and bar candidates. By enforcing geometric consistency across prediction levels and decoding with beam search and learned temporal priors, CODA improves tracking precision over prior methods that rely on independent, frame-wise detection. The silence-driven break mode provides, to our knowledge, the first explicit jump recovery mechanism for image-based score following, handling repeats, D.C., coda jumps, and performer errors through a single unified mechanism.
 
Several limitations point to important directions for future work. First, like all prior image-based score following methods, CODA is trained and evaluated exclusively on solo piano music from the MSMD dataset. The limited size and variation of this dataset can lead to overfitting, particularly for visually or acoustically unusual passages such as extremely slow pieces, dense trills, or arpeggios --- failure modes also observed in prior work~\cite{henkel2021real}.
Scaling to larger and more diverse datasets, potentially through semi-supervised approaches that leverage coarsely annotated data, is essential for improving robustness. Second, extending beyond piano to other instruments and ensembles remains an open challenge: polyphonic mixtures, varying timbres, and multi-staff scores (e.g., orchestral parts) introduce both visual and acoustic complexities that the current pipeline does not address. Third, our evaluation
is limited to synthetic and re-rendered audio with typeset score images. Evaluating on scanned real-world scores paired with commercial recordings, as partially explored by~\cite{henkel2021real}, would provide a more realistic assessment of generalization. Making such datasets publicly available would benefit the broader research community.
 
More broadly, real-time score following from images has the potential to make interactive music applications --- automatic page turning, synchronized score display, and live accompaniment --- accessible without requiring any symbolic score representation. We hope that the cascaded selection formulation and jump recovery mechanism introduced in this work provide a useful foundation for future systems that can operate reliably across the full diversity of real-world musical performance.
