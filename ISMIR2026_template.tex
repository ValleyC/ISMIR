...
Figure~\ref{fig:overview} illustrates the overall architecture. CODA has two input streams: audio and visual.
 
The audio stream is converted into a 78-dimensional log-filterbank at 20~frames per second. A two-layer causal Mamba encoder~\cite{gu2024mamba} processes each frame, producing a global conditioning vector~$z_t$ that summarizes the audio context up to frame~$t$, and a buffer of recent hidden states~$H_t$ that serves as the key--value source for cross-attention.
 
The score page image is processed by a convolutional backbone with FiLM conditioning at deeper stages, followed by a Feature Pyramid Network (FPN) that produces a single stride-8 feature map~$F$, shared across all three cascade stages.
...
To evaluate CODA's break mode, we construct a jump-augmented test set by inserting 1--6 synthetic jumps per piece (scaled by duration) into the 94 MSMD test pieces, with three variants each. At each jump, a silence gap precedes playback from a randomly selected destination system. Ground-truth labels follow the destination, so a correct tracker must relocalize after each silence.
 
We report three metrics on post-jump segments: \emph{system recovery rate} (fraction of jumps where the correct system is identified within 1.0 and 2.0\,s), \emph{mean recovery latency} (frames to first correct system prediction), and \emph{post-jump tracking error} (${\leq}\,1.0$\,s threshold in the 5\,s window after each jump).
...
Table~\ref{tab:jump} shows the results. CYOLO-SB has no recovery mechanism and continues from its last position after each jump, yielding low recovery rates. CODA without break mode also struggles because learned transition penalties resist large jumps. Enabling break mode substantially improves recovery by suppressing priors and widening the beam during detected silence.
