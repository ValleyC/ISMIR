...
All training and experiments are conducted on a single NVIDIA RTX A6000 GPU (48\,GB) with an Intel Xeon Gold 5218R CPU and 64\,GB RAM. At inference, CODA processes each audio frame in 12.8\,ms (78.1\,fps), within the 50\,ms real-time budget at 20\,fps.
...
Table~\ref{tab:jump} shows the results. CYOLO-SB has no recovery mechanism and continues from its last committed position, yielding low recovery rates on both subsets. CODA without break mode also struggles because learned transition penalties resist large position changes. Enabling break mode substantially improves recovery by suppressing priors and widening the beam during detected silence. On the repeat subset, CODA recovers the correct system within 1\,s in 78\% of jumps. Recovery is higher on the repeat subset than the random subset (.78 vs.\ .64 at 1\,s), suggesting that returning to previously heard material provides stronger audio evidence for relocalization.
...
The largest degradation comes from removing the cascade ($-.045$ at ${\leq}\,0.10$\,s, bar accuracy drops from .975 to .931), confirming that enforced geometric consistency is the most important component. Beam search and cross-attention contribute the next-largest gains ($-.026$ and $-.019$ at ${\leq}\,0.10$\,s, respectively).
