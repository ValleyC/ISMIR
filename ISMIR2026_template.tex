...
To fairly evaluate the model's performance on jump recovery, a benchmark with fixed but various discontinuities is needed. Accordingly, we construct a jump-augmented test benchmark from the 94 MSMD test pieces. We manually annotated the repeat structure of each piece (repeat barlines, da capo, binary form, etc.). The testset is partitioned into two subsets. The \emph{repeat subset} contains pieces with written repeat structures, where jumps follow the annotated performance order. The \emph{random subset} contains pieces without repeats, each receiving at least 3 diverse random jumps as a stress test.
