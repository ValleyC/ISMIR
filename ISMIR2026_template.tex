...
Let $H_t = [h_{t-L+1}, \ldots, h_t] \in \mathbb{R}^{L \times d}$ denote the buffer of the most recent~$L$ Mamba hidden states, where $d$ is the hidden dimension of the Mamba encoder. For each candidate region~$r$ (a system or bar bounding box from the score layout metadata), let $f_r$ denote the extracted visual features for~$r$. The pipeline proceeds as follows.
